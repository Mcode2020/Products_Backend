...
const { Invoice, Order, Product, Inventory, Vendor } = models;
  ...
  private async generateInvoice(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (!vendor) {
       ...
       console.log(vendor,"111111111111111111111111")
      ...
      doc.fontSize(14).text(`Name: ${vendor.name}`);
      doc.text(`Username: ${vendor.username}`);
      doc.text(`Mobile: ${vendor.mobileNumber}`);
      doc.text(`Location: ${vendor.location.address}, ${vendor.location.city}, ${vendor.location.state}, ${vendor.location.country}`);
      if (vendor.notes) {
        doc.text(`Notes: ${vendor.notes}`);
      }
      doc.text(`Active: ${vendor.active ? "Yes" : "No"}`);
      doc.moveDown();
